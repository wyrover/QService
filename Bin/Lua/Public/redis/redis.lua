...
local function create_connection(parameters)
    ...
    local perform_connection, sockets

    if parameters.scheme == 'unix' then
        perform_connection, sockets = connect_unix, require('socket.unix')
        assert(sockets, 'your build of LuaSocket does not support UNIX domain sockets')
    else
        if parameters.scheme then
            local scheme = parameters.scheme
            assert(scheme == 'redis' or scheme == 'tcp', 'invalid scheme: '..scheme)
        end
        perform_connection, sockets = connect_tcp, socket.tcp
    end

    return perform_connection(sockets(), parameters)
end
...
function redis.connect(...)
    ...
    local sockets = create_connection(merge_defaults(parameters))
    local client = create_client(client_prototype, sockets, commands)
